...
func TestSelectiveAccessDescriptor(t *testing.T) {
	var a SelectiveAccessDescriptor = *CreateSelectiveAccessDescriptor(AccessSelectorEntry, []uint32{0, 5})
	t1 := a.Encode()
	result := []byte{2, 4, 6, 0, 0, 0, 0, 6, 0, 0, 0, 5, 18, 0, 0, 18, 0, 0}
	...
	t2 := b.Encode()
	result = []byte{2, 4, 2, 4, 18, 0, 8, 9, 6, 0, 0, 1, 0, 0, 255, 15, 2, 18, 0, 0, 25, 7, 228, 1, 1, 3, 10, 0, 0, 0, 0, 0, 0, 25, 7, 228, 1, 1, 3, 11, 0, 0, 0, 0, 0, 0, 1, 0}
